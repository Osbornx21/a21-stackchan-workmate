...
func TestValidatePlatformIORejectsFirmwareVersionMismatch(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "platformio.ini")
	content := `[env:a21_stackchan_cores3]
platform = espressif32@7.0.1
...
`
	...
	err := validatePlatformIO(path, Manifest{FirmwareID: "a21-stackchan", Version: "0.1.0", Board: "m5stack-cores3"})
	if err == nil {
		t.Fatalf("expected mismatched firmware version build flag to be rejected")
	}
}

func TestValidatePlatformIORejectsFirmwareIDMismatch(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "platformio.ini")
	content := `[env:a21_stackchan_cores3]
platform = espressif32@7.0.1
board = m5stack-cores3
build_flags =
  -D A21_FIRMWARE_ID=\"a21-stackchan-lite\"
  -D A21_FIRMWARE_VERSION=\"0.1.0\"
  -D A21_FIRMWARE_BOARD=\"m5stack-cores3\"
  -D A21_GATEWAY_HOST=\"10.21.0.1\"
  -D A21_GATEWAY_PORT=21080
...
`
	...
	err := validatePlatformIO(path, Manifest{FirmwareID: "a21-stackchan", Version: "0.1.0", Board: "m5stack-cores3"})
	if err == nil {
		t.Fatalf("expected mismatched firmware id build flag to be rejected")
	}
}

func TestValidatePlatformIORejectsNativeEnvironmentFirmwareVersionMismatch(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "platformio.ini")
	content := `[env:a21_stackchan_cores3]
platform = espressif32@7.0.1
...
`
	...
	err := validatePlatformIO(path, Manifest{FirmwareID: "a21-stackchan", Version: "0.1.0", Board: "m5stack-cores3"})
	if err == nil {
		t.Fatalf("expected mismatched native firmware version build flag to be rejected")
	}
}

...
func buildStackChanFastCompanionTurnReport(ctx context.Context, options stackChanFastCompanionTurnOptions) (stackChanFastCompanionTurnReport, error) {
	...
	for turn := 1; turn <= options.Repeat; turn++ {
		...
		if receipt.BargeInStopMS >= 0 && options.ListenSource == "stackchan_mic" {
			bargeSamples = append(bargeSamples, time.Duration(receipt.BargeInStopMS*1000)*time.Microsecond)
		}
	}
	if len(ackSamples) > 0 {
...
func runStackChanFastCompanionSingleTurn(ctx context.Context, options stackChanFastCompanionTurnOptions, turn int, generatedAtMS int64) (stackChanFastCompanionTurnReceipt, error) {
	...
	if options.ListenSource == "stackchan_mic" {
		bargeInStopMS, err := measureStackChanFastCompanionBargeIn(ctx, options.GatewayURL, options.DeviceID, traceID, sessionID, uint64(turn))
		if err != nil {
			receipt.Findings = append(receipt.Findings, "barge-in stop evidence missing")
			return receipt, err
		}
		receipt.BargeInStopMS = bargeInStopMS
		receipt.TraceMarkers = append(receipt.TraceMarkers, "barge_in.detected", "playback.stop")
	}

...
func BaselineFrontEndCandidates() []FrontEndCandidate {
	return []FrontEndCandidate{
		...
		},
		{
			...
			Status:            "adapter_boundary_ready_not_product_accepted",
			Available:         false,
			Placeholder:       true,
			PlaceholderReason: "adapter_boundary_ready_but_model_runtime_not_installed_no_office_noise_cpu_memory_or_physical_evidence",
			A21Fit:            "Good candidate for speech/non-speech quality behind an injectable A21 adapter if Gateway CPU and model packaging remain simple.",
			Risks: []string{

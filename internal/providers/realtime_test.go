...
func TestRealtimeWebSocketPlanFromEnvBuildsDoubaoS2SPlanWithoutLeakingSecrets(t *testing.T) {
	report := RealtimeWebSocketPlanFromEnv([]string{
	...
	if report.Provider != "doubao_realtime" {
		t.Fatalf("provider = %q, want doubao_realtime", report.Provider)
	}
	if report.Status != ProviderSmokeReady {
		t.Fatalf("status = %q, want ready", report.Status)
	}
...
func TestRealtimeWebSocketPlanFromEnvSkipsDoubaoS2SWhenResourceMissing(t *testing.T) {
	report := RealtimeWebSocketPlanFromEnv([]string{
		"A21_PROVIDER_PRIMARY=doubao_realtime",
		"A21_DOUBAO_API_KEY=sk-a21-secret",
		"A21_DOUBAO_APP_ID=app-a21-secret",
	...
	if !stringSliceContains(report.MissingEnv, "A21_DOUBAO_RESOURCE_ID") {
		t.Fatalf("missing env lacks resource id: %#v", report.MissingEnv)
	}
}

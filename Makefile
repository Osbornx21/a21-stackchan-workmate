...
A21_STACKCHAN_OFFICIAL_BASELINE_FLASH_CONFIRM ?=
A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_FIRST_BOOT_CONFIG ?=
A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_FIRST_BOOT_ARG := $(if $(filter 1 true TRUE yes YES,$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_FIRST_BOOT_CONFIG)),--first-boot-config,)
...
A21_STACKCHAN_PRODUCT_RECOVERY_GATEWAY_URL ?= http://47.103.57.217
A21_STACKCHAN_PRODUCT_RECOVERY_DEVICE_ID ?= 44:1b:f6:e2:6a:60
A21_STACKCHAN_PRODUCT_RECOVERY_POST_CHECK_DELAY_MS ?= 8000
...
A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_VERSION ?= 1
...
server-side-readiness-bundle:
	go run ./cmd/a21 server-side-readiness-bundle --gateway-url "$${A21_GATEWAY_URL:-http://127.0.0.1:21080}" --device-id "$${A21_DEVICE_ID:-stackchan-001}" --use-latest-reports --output-dir reports

server-side-readiness-collect:
	go run ./cmd/a21 server-side-readiness-bundle --gateway-url "$${A21_GATEWAY_URL:-http://127.0.0.1:21080}" --device-id "$${A21_DEVICE_ID:-stackchan-001}" --use-latest-reports --collect-missing --output-dir reports
...
provider-evidence-package:
	...
	go run ./cmd/a21 provider-evidence-package --input-dir "$(A21_PROVIDER_EVIDENCE_INPUT_DIR)" --output-dir "$${A21_PROVIDER_EVIDENCE_OUTPUT_DIR:-reports}"

provider-evidence-import:
	@test -n "$(A21_PROVIDER_EVIDENCE_BUNDLE)" || (echo "A21_PROVIDER_EVIDENCE_BUNDLE is required"; exit 2)
	go run ./cmd/a21 provider-evidence-import --bundle "$(A21_PROVIDER_EVIDENCE_BUNDLE)" --output-dir "$${A21_PROVIDER_EVIDENCE_OUTPUT_DIR:-reports}"
...
local-asr-streaming-smoke:
	go run ./cmd/a21 local-asr-streaming-smoke --output-dir reports

streaming-tts-runtime-smoke:
	go run ./cmd/a21 streaming-tts-runtime-smoke --output-dir reports
...
stackchan-official-baseline-flash-plan:
	...
	go run ./cmd/a21 stackchan-official-baseline-flash --build-dir "$(A21_STACKCHAN_OFFICIAL_BUILD_DIR)" --idf-export "$(A21_IDF_EXPORT)" --port "$(A21_UPLOAD_PORT)" --output-dir reports

stackchan-official-baseline-flash-execute:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_BASELINE_FLASH_CONFIRM)" || (echo "A21_STACKCHAN_OFFICIAL_BASELINE_FLASH_CONFIRM is required"; exit 2)
	go run ./cmd/a21 stackchan-official-baseline-flash --execute --build-dir "$(A21_STACKCHAN_OFFICIAL_BUILD_DIR)" --idf-export "$(A21_IDF_EXPORT)" --port "$(A21_UPLOAD_PORT)" --confirm "$(A21_STACKCHAN_OFFICIAL_BASELINE_FLASH_CONFIRM)" --output-dir reports
...
stackchan-product-recovery:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	go run ./cmd/a21 stackchan-product-recovery --gateway-url "$(A21_STACKCHAN_PRODUCT_RECOVERY_GATEWAY_URL)" --device-id "$(A21_STACKCHAN_PRODUCT_RECOVERY_DEVICE_ID)" --direct-source-ip "$${A21_DIRECT_SOURCE_IP:-}" --upload-port "$(A21_UPLOAD_PORT)" --reports-dir reports --output-dir reports

stackchan-product-recovery-execute:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_APP_FLASH_CONFIRM)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_APP_FLASH_CONFIRM is required"; exit 2)
	go run ./cmd/a21 stackchan-product-recovery --gateway-url "$(A21_STACKCHAN_PRODUCT_RECOVERY_GATEWAY_URL)" --device-id "$(A21_STACKCHAN_PRODUCT_RECOVERY_DEVICE_ID)" --direct-source-ip "$${A21_DIRECT_SOURCE_IP:-}" --upload-port "$(A21_UPLOAD_PORT)" --reports-dir reports --output-dir reports --execute-flash --confirm "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_APP_FLASH_CONFIRM)" --build-dir "$(A21_STACKCHAN_OFFICIAL_BUILD_DIR)" --idf-export "$(A21_IDF_EXPORT)" --wait-rom-timeout-seconds "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WAIT_ROM_TIMEOUT_SECONDS)" --post-check-delay-ms "$(A21_STACKCHAN_PRODUCT_RECOVERY_POST_CHECK_DELAY_MS)"

a21-stackchan-official-xiaozhi-compatible-nvs-plan:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL is required"; exit 2)
	go run ./cmd/a21 a21-stackchan-official-xiaozhi-compatible-nvs --idf-export "$(A21_IDF_EXPORT)" --port "$(A21_UPLOAD_PORT)" --ota-url "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL)" --websocket-url "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL)" --websocket-version "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_VERSION)" $(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_FIRST_BOOT_ARG) --output-dir reports

a21-stackchan-official-xiaozhi-compatible-nvs-execute:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL is required"; exit 2)
	@test -n "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_CONFIRM)" || (echo "A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_CONFIRM is required"; exit 2)
	go run ./cmd/a21 a21-stackchan-official-xiaozhi-compatible-nvs --execute --idf-export "$(A21_IDF_EXPORT)" --port "$(A21_UPLOAD_PORT)" --ota-url "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_OTA_URL)" --websocket-url "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_URL)" --websocket-version "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_WEBSOCKET_VERSION)" $(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_FIRST_BOOT_ARG) --confirm "$(A21_STACKCHAN_OFFICIAL_XIAOZHI_COMPATIBLE_NVS_CONFIRM)" --output-dir reports
...
wake-word-firmware-package:
	@test -n "$(A21_WAKE_WORD_FIRMWARE_PLAN)" || (echo "A21_WAKE_WORD_FIRMWARE_PLAN is required"; exit 2)
	@test -n "$(A21_XIAOZHI_FIRMWARE_BUILD_DIR)" || (echo "A21_XIAOZHI_FIRMWARE_BUILD_DIR is required"; exit 2)
	go run ./cmd/a21 wake-word-firmware-package --plan "$(A21_WAKE_WORD_FIRMWARE_PLAN)" --build-dir "$(A21_XIAOZHI_FIRMWARE_BUILD_DIR)" --build-receipt "$(A21_WAKE_WORD_BUILD_RECEIPT_OUTPUT_DIR)/a21-wake-word-build.json" --output-dir "$(A21_WAKE_WORD_FIRMWARE_OUTPUT_DIR)" --commit $$(git rev-parse --short=12 HEAD)

wake-word-physical-proof:
	@test -n "$(A21_WAKE_WORD_GUARDED_FLASH_REPORT)" || (echo "A21_WAKE_WORD_GUARDED_FLASH_REPORT is required"; exit 2)
	go run ./cmd/a21 wake-word-physical-proof --physical-device-online --firmware-flash-executed --guarded-flash-report "$(A21_WAKE_WORD_GUARDED_FLASH_REPORT)" --operator-observed --wake-phrase-matched --false-wake-rejected --stock-wake-rejected --output-dir reports

wake-word-physical-acceptance:
	@test -n "$(A21_WAKE_WORD_FIRMWARE_PACKAGE_REPORT)" || (echo "A21_WAKE_WORD_FIRMWARE_PACKAGE_REPORT is required"; exit 2)
	@test -n "$(A21_WAKE_WORD_PHYSICAL_PROOF_REPORT)" || (echo "A21_WAKE_WORD_PHYSICAL_PROOF_REPORT is required"; exit 2)
	go run ./cmd/a21 wake-word-physical-acceptance --package-report "$(A21_WAKE_WORD_FIRMWARE_PACKAGE_REPORT)" --proof-report "$(A21_WAKE_WORD_PHYSICAL_PROOF_REPORT)" --output-dir reports

xiaozhi-firmware-flash-plan:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_XIAOZHI_FIRMWARE_BUILD_DIR)" || (echo "A21_XIAOZHI_FIRMWARE_BUILD_DIR is required"; exit 2)
	go run ./cmd/a21 xiaozhi-firmware-flash --build-dir "$(A21_XIAOZHI_FIRMWARE_BUILD_DIR)" --idf-export "$(A21_IDF_EXPORT)" --port "$(A21_UPLOAD_PORT)" --output-dir reports

xiaozhi-firmware-flash-execute:
	@test -n "$(A21_UPLOAD_PORT)" || (echo "A21_UPLOAD_PORT is required"; exit 2)
	@test -n "$(A21_XIAOZHI_FIRMWARE_BUILD_DIR)" || (echo "A21_XIAOZHI_FIRMWARE_BUILD_DIR is required"; exit 2)
	@test "$(A21_XIAOZHI_FIRMWARE_FLASH_CONFIRM)" = "WRITE_A21_XIAOZHI_FIRMWARE" || (echo "A21_XIAOZHI_FIRMWARE_FLASH_CONFIRM=WRITE_A21_XIAOZHI_FIRMWARE is required"; exit 2)

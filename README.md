...
`A21_LAN_TARGET=a21-gateway=127.0.0.1:21080 A21_LAN_SAMPLES=5 make lan-probe` writes an ignored `reports/a21-lan-probe-*.json` direct TCP reachability and jitter report. Use it in Shanghai for Gateway, StackChan-adjacent LAN endpoints, and the A21 V21 adapter boundary. It does not use proxy settings and never stores proxy URLs or credentials.

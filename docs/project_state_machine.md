...
| T-ALIYUN-CLOUD-GATEWAY-APP-REACHABILITY-001: Cloud Gateway application reachability | Public cloud TCP ports are reachable, but HTTP/HTTPS app requests to `47.103.57.217` and `47.103.57.217:21081` currently return empty replies or TLS syscall errors from this control network; SSH closes before authentication | Use ECS/control-plane access or cloud console to confirm `a21-gateway` and Caddy runtime health, restore public `/healthz`, `/xiaozhi/ota/`, `/v1/devices`, and `/v1/xiaozhi`, then re-poll device `44:1b:f6:e2:6a:60` and collect physical evidence. |

...
8. `T-XIAOZHI-STREAMING-ASR-PROVIDER-001`
   - Current phase: static provider readiness gate landed and is correctly red.
     The best current selected chain has real text streaming but still has
     Sherpa ASR WAV/batch and Iflytek TTS WAV/file boundaries.
   - Next action: implement one real streaming ASR adapter first, preferably a
     long-lived Sherpa streaming session or Iflytek IAT streaming session, then
     rerun `xiaozhi-streaming-provider-readiness` and physical

...
| Firmware candidate | `S5J-APP-PRELOAD-DIRECT-XIAOZHI-HOTFIX-CANDIDATE` | Commit `a986d6b` request-start physical flash regressed to the welcome/setup trap. Current hotfix keeps official app install preload but starts Xiaozhi directly before any Mooncake update/setup frame can render. It still sets codec volume `92`, keeps `A21_STACKCHAN_KEEP_CONTROL_CHANNEL`, opens the stock Xiaozhi WebSocket quietly while idle, shows `紫悦` connecting/ready copy, and keeps the 7s no-speech device-side listen timeout. Build report `reports/a21-stackchan-official-baseline-20260603-041402-1780431242345821000.json` passed with app SHA-256 `7ff81bb0e564e020128e02068bc7d83b36f90c21de8cbd3d4b89b1dd1d6e9cf3`; flash/physical proof are pending. | `S5K-APP-PRELOAD-DIRECT-XIAOZHI-FLASHED` |

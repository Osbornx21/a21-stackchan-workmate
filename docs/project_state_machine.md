...
| Firmware candidate | `S5K-APP-PRELOAD-DIRECT-XIAOZHI-FLASHED` | Commit `a986d6b` request-start physical flash regressed to the welcome/setup trap. Hotfix commit `9ba8bc1` keeps official app install preload but starts Xiaozhi directly before any Mooncake update/setup frame can render. It still sets codec volume `92`, keeps `A21_STACKCHAN_KEEP_CONTROL_CHANNEL`, opens the stock Xiaozhi WebSocket quietly while idle, shows `紫悦` connecting/ready copy, and keeps the 7s no-speech device-side listen timeout. Build report `reports/a21-stackchan-official-baseline-20260603-041402-1780431242345821000.json` passed with app SHA-256 `7ff81bb0e564e020128e02068bc7d83b36f90c21de8cbd3d4b89b1dd1d6e9cf3`; flash execute `reports/a21-stackchan-official-xiaozhi-compatible-flash-20260603-041636-1780431396539566000.json` passed on `/dev/cu.usbmodem1101`; device `44:1b:f6:e2:6a:60` reconnected `online`; runtime speaker volume `100` was delivered on trace `a21-trace-direct-preload-hotfix-volume-1780431400`. Operator still needs to confirm screen/wake behavior physically. | `S5L-APP-PRELOAD-DIRECT-XIAOZHI-PHYSICAL-ACCEPTED` |
...
| T-STACKCHAN-APP-PRELOAD-NO-WELCOME-001b: Park after direct Xiaozhi start | Flashed, physical validation pending | Operator reported the welcome/setup page reappeared after the multi-wake product flash even though the binary contained the direct autostart log. Source inspection proved the false assumption: `Hal::startXiaozhi()` starts Xiaozhi tasks and returns, so `main.cpp` could continue into the Mooncake main loop and AppLauncher setup worker. The overlay now parks `app_main` after direct `GetHAL().startXiaozhi()` with `GetHAL().feedTheDog()` and `GetHAL().delay(1000)`, before the Mooncake main loop. Focused app tests, `git diff --check`, and `make verify` passed. Product build report `reports/a21-stackchan-official-baseline-20260603-054837-1780436917458986000.json` passed with app SHA-256 `e3758cffdc294e74220f5288a33306d4aace164279bea6acb3a2cea5af17e1c3`; no-write plan `reports/a21-stackchan-official-xiaozhi-compatible-flash-20260603-054848-1780436928156547000.json` passed; guarded flash execute `reports/a21-stackchan-official-xiaozhi-compatible-flash-20260603-054954-1780436994114555000.json` passed on `/dev/cu.usbmodem1101` from clean commit `e694550f1739`. Device `44:1b:f6:e2:6a:60` reconnected online, runtime speaker volume `100` was delivered by trace `a21-trace-no-welcome-park-volume-1780437008`, and post-flash trace events since `1780436933516` contained only `xiaozhi.hello.received=2` with no automatic `listen.start`. |
...
8. `T-XIAOZHI-STREAMING-ASR-PROVIDER-001`
   - Current phase: static provider readiness gate landed and is correctly red;
     ...
     but real helper/model runtime proof is not executed yet. With redacted
     helper/model env present, ASR+LLM can be marked ready while Iflytek TTS
     still blocks as a WAV/file boundary.
   - Next action: implement or connect the long-lived Sherpa streaming helper
     for real runtime proof, then implement streaming TTS chunk emission before
     rerunning physical `xiaozhi-realtime-parity`.

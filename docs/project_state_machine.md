...
| T-STACKCHAN-PRODUCT-FLASH-CHINANET-NVS-001: Product app flash and first ChinaNet NVS refresh | Completed guarded hardware writes, SSID correction pending | In the foreground hardware-window branch `codex/a21-hardware-window-20260604-internal-test4-local-lan-nvs` at clean commit `522c5c28d335`, the control tower used only the product lane and flashed `/tmp/a21-stackchan-official-build/a21-stackchan-official-xiaozhi-compatible.bin` on `/dev/cu.usbmodem1101`; execute report `reports/a21-stackchan-official-xiaozhi-compatible-flash-20260604-195734-1780574254123911000.json` passed with app SHA-256 `e66a41ef486b866b076746bd064af2e3afb75e0a316515921bbc681b89fb36a8`. Product NVS execute report `reports/a21-stackchan-official-xiaozhi-compatible-nvs-20260604-195752-1780574272065931000.json` passed with `wifi_credentials_written=true`, `mutated_entry_count=5`, `servo_calibration_present=true`, OTA `http://47.103.57.217/xiaozhi/ota/`, and WebSocket `ws://47.103.57.217/v1/xiaozhi`, but it used the first typed SSID spelling. Serial startup confirmed the A21 official Xiaozhi-compatible app booted and entered Wi-Fi station mode, but the device logged `No AP found`; Mac-side explicit join to that first typed SSID spelling also failed. Operator screenshot later clarified that the visible SSID is `ChinaNet-N6e3` with no spaces around the hyphen, so the next hardware step is a corrected product NVS write. Public Gateway probes to `47.103.57.217` on `80` and `21081` TCP-connect but return empty HTTP replies, and SSH closes before authentication. This is hardware write completion, not PRD acceptance. |
...
| T-ALIYUN-CLOUD-GATEWAY-APP-REACHABILITY-001: Cloud Gateway application reachability | Public cloud TCP ports are reachable, but HTTP/HTTPS app requests to `47.103.57.217` and `47.103.57.217:21081` currently return empty replies; SSH to `root@47.103.57.217` closes before authentication. The latest Web/App professional-query commit is therefore not deployable from this Mac right now. | Use ECS/control-plane access or cloud console to confirm `a21-gateway` and Caddy runtime health, restore public `/healthz`, `/xiaozhi/ota/`, `/v1/devices`, and `/v1/xiaozhi`, then deploy the latest Gateway commit and re-poll device `44:1b:f6:e2:6a:60`. |
| T-STACKCHAN-CHINANET-SSID-SPELLING-001: Correct requested Wi-Fi SSID spelling | Product NVS was written with the first typed SSID spelling and cloud endpoints, but serial startup logged `No AP found`; operator screenshot shows the visible SSID is `ChinaNet-N6e3` with no spaces around the hyphen. | Rewrite product NVS with the corrected SSID spelling, hard-reset the device, confirm `got ip`, then restore/check public Gateway and collect `/v1/devices` plus physical `/v1/xiaozhi` evidence. |

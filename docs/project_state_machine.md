...
Current total state: `S-INTERNAL-TEST4-ROLEPLAY-MEMORY-CONTROL-READY-WORKSPACE-JOB-SKELETON-READY-CLOUD-UPLOAD-INDEX-EXECUTION-PLANNED-PHYSICAL-PENDING`
...
| T-INTERNAL-TEST4-CLOUD-MODE-AND-KNOWLEDGE-WORKSPACE-001: Roleplay/professional v2 and cloud workspace | Active roleplay memory + workspace job skeleton cut | Plan `docs/plans/2026-06-04-internal-test4-cloud-mode-and-knowledge-workspace.md` scopes the work. A21 internal test 4 makes `roleplay` the default user-facing embodied mode with role/personality, memory hints, and voice-clone selection, keeps `professional` as the explicit evidence/V21 path, and defines the cloud workspace product form: upload documents, query public-only or personal+public resources, and consult through web/app or A21 hardware without putting provider/V21 secrets or documents on StackChan. Current Gateway runtime exposes `/v1/voice-modes`, `/v1/roleplay-profile`, `/v1/professional-workspace`, and `/v1/workspace-upload-jobs`; accepts `dialogue` as a compatibility alias; lets `/v1/roleplay-profile` set/clear bounded runtime `memory_hints` while responses/traces show only readiness/count/finding metadata; redacts roleplay prompt/memory/transcript/provider/V21 data from fast-companion summaries and traces; sends redacted `device_id`/`user_id`/`workspace_id`/`query_scope` fields through the professional V21 adapter v2 contract; and supports no-execute workspace job create/poll/fail/retry/delete metadata. |
...
| T-V21-A21-V2-WORKSPACE-QUERY-SCOPE-NATIVE-001: V21 native A21 v2 workspace query-scope contract | Prepared for scoped V21 worker dispatch | Plan `docs/plans/2026-06-04-v21-a21-v2-workspace-query-scope-native-contract.md` scopes a V21-repo worker. A21 already sends v2 professional scope fields and can wrap V21 retrieval results with redacted `source_scope_counts` and `workspace_status`. The next V21 worker should make `/internal/v1/knowledge/voice-query` accept and report A21 v2 fields natively, while truthfully blocking or marking pending any personal/public ACL enforcement that current V21 schema cannot prove. The worker must not touch A21 code, A21 Gateway runtime, StackChan hardware, firmware, or existing dirty V21 LAN discovery changes. |
...
- `T-V21-A21-V2-WORKSPACE-QUERY-SCOPE-NATIVE-001`
  - Current phase: A21 control plan written and ready for V21 scoped worker
    dispatch.
  - Plan:
    `docs/plans/2026-06-04-v21-a21-v2-workspace-query-scope-native-contract.md`.
  - Next action: run a V21 worker in
    `/Users/jiyurun/Documents/v21-knowledge-platform` to add native
    `/internal/v1/knowledge/voice-query` support for A21 v2 fields
    `device_id`, `user_id`, `workspace_id`, and `query_scope`, plus safe
    `source_scope_counts` and `workspace_status` response metadata.
  - Boundary: do not touch A21 code, V21 LAN discovery dirty work, provider
    execution, hardware, firmware, Gateway runtime, report cleanup, or
    personal/public ACL claims that current V21 evidence cannot prove.

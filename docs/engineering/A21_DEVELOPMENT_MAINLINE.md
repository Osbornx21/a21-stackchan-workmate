...
Provider spine progress: `ProviderProfile` is now the single registry shape, and the built-in catalog covers the PRD reference profiles `siliconflow`, `deepseek`, `stepfun`, `bailian_dashscope`, `moonshot`, `volcengine_ark`, `local_ollama`, `local_vllm`, `openai_realtime`, `doubao_realtime`, `doubao_tts_realtime`, `hermes_agent`, and `mimo_agent`. This is readiness visibility, not broad execution authorization: the P0 route-eligible set remains intentionally narrow at `mock`, `deepseek`, `stepfun`, and `local_ollama`. Baidu/Huawei candidates remain blocked and redacted. StepFun is the current launch-policy OpenAI-compatible cloud text-stream profile once the ECS operator has provisioned its required A21 env names and the public voice-chain selector is set to `stepfun`; DeepSeek remains the fallback cloud text-stream profile with a default `deepseek-chat` model and the lab key name `A21_LAB_DEEPSEEK_API_KEY`; `local_ollama` is the P0 local-fallback text-stream lane through Ollama `/api/chat`, with explicit local base URL and model env requirements. `provider-smoke --provider stepfun --stream --repeat N`, `provider-smoke --provider deepseek --stream --repeat N`, and `provider-smoke --provider local_ollama --stream --repeat N` record redacted first-byte, first-content, total-duration, fallback, trace, and metric evidence while preserving the existing no-network-without-`--execute` rule. OpenAI-compatible text-stream candidates that are not route-eligible, such as SiliconFlow, DashScope, Moonshot, Volcengine Ark, or hot-plug `a21_` profiles, may still run explicit compatibility smoke when configured, but their reports remain `route_eligible=false` and cannot close product readiness unless a separate promotion explicitly marks them route-eligible. `provider-compat-matrix` is the ASR/LLM/TTS local/cloud burn-down receipt: it ingests the 5080 full provider summary, local ASR/TTS reports, provider-smoke reports, and redacted `a21.provider_audio_smoke.v1` cloud audio reports. Product readiness no longer treats env-only provider configuration as real readiness: it accepts only an executed, current-provider, non-mock, route-eligible text-stream `a21.provider_smoke.v1` report with `--provider-smoke-report` or `--use-latest-reports`, and still keeps V21, physical StackChan, wake-word, and voice acceptance gates separate. Other provider families and profiles stay as known non-route-eligible readiness or plan boundaries until a follow-up slice promotes them with evidence.

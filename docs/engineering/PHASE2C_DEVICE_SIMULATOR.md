...
- Receives Gateway `audio.playback.chunk` downlink envelopes and displays downlink count, buffered chunk count, and active `stream_id`
...
Current playback support shows Gateway downlink chunk flow and plays local mock feedback only. It does not decode PCM chunks or play real Gateway/Provider TTS audio yet.
...
- wav upload fixtures
- real downlink PCM decode/playback once the Gateway emits provider TTS audio chunks

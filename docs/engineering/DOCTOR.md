...
`stackchan-accept --check xiaozhi-half-duplex` is the stock Xiaozhi product-firmware half-duplex trace gate. It does not command `/v1/devices/control`, does not require diagnostic `runtime_echo` mic counters, and does not require `microphone=diagnostic_probe_m5unified_i2s_capture`. It reads the latest device `last_trace_id` / `last_session_id` from `/v1/devices` when explicit trace fields are omitted, then cross-checks `/v1/traces` and `/v1/audio/recent` for real stock WebSocket evidence: physical device online, stock or isolated-debug Xiaozhi profile, Opus decode, PCM mic ingress, VAD/listen lifecycle, TTS Opus downlink, playback observation when available, and barge-in stop markers. It writes `reports/a21-xiaozhi-half-duplex-acceptance-YYYYMMDD-HHMMSS.NNNNNNNNN.json` with `hardware_acceptance_scope=stock_xiaozhi_mic_to_tts_downlink`, `diagnostic_mic_probe_required=false`, and `prd_accepted=false`. Passing status is `physical_review_required`; it still needs operator or instrument review and separate wake-word proof before any full PRD green claim. `a21 xiaozhi-physical-prd-review` is the follow-on report-only promote gate: with `ACCEPT_A21_XIAOZHI_PHYSICAL_PRD`, it consumes matching physical and half-duplex reports, verifies playback-start, audible observation, mic, barge-in stop, stop_done, and redaction, then writes an accepted `a21.xiaozhi_physical_evidence.v1` report for `product-readiness`.

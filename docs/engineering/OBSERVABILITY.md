...
`runtime_echo` is emitted by firmware after it applies screen, motion, and RGB state. Current stable keys are `screen`, `servo_y`, and `rgb`. Diagnostic mic-probe firmware also emits string counters for capture/send debugging, including mic frame count, driver errors, skip reasons, queue depth, queue drops, and audio-WebSocket sent frame count. This is useful for capability evidence derivation and driver-state debugging, but physical office acceptance still needs separate visible/audible/operator or instrument observations.

...
- audio WebSocket begin gating after Gateway connection, mock `audio.frame` envelope construction, ack control-event parsing, mock `audio.playback.chunk` downlink parsing, full 20 ms 16 kHz PCM base64 payload capacity, fixed 640-byte PCM decode, bounded playback buffering, M5 speaker pump queue gating, and send rejection while disconnected

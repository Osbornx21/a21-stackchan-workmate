...
- Control HEAD: `1c6de691437a feat(control): enforce A21 hardware write guard`.
...
1. StackChan hardware mainline diagnostic consolidation.
...
2. Provider Spine / text-stream hot plug.
   ...
   - Gate: start only after the control tower either closes or deliberately
     defers the hardware diagnostic consolidation slice.
   ...
   - Forbidden: provider keys in docs/reports/logs, provider URLs in firmware,
     hidden proxy inheritance, or adding Baidu/Huawei as candidates.

...
### Provider Latency Report v2 Implementation Thread

Opened by the control tower after accepting the post-AgentTask PRD next-slice
audit.

Evidence:

- Implementation thread: `019e7c65-5abb-77d3-9e43-04b53147a1ae`.
- Implementation thread title:
  `A21 Provider Latency Report v2：Implementation`.
- Implementation worktree:
  `/Users/jiyurun/.codex/worktrees/45f5/New project`.
- Starting branch: `codex/a21-integration-governance-slices`.
- Starting control HEAD:
  `2f34854 docs(control): track post-agent prd audit`.
- Target branch: `codex/a21-provider-latency-report-v2`.
- Scope: TDD hardening of `provider-latency-bench` report-contract and docs
  for Fast Companion metric-shape comparison, without opening runtime or
  execution surfaces.
- Expected files: `internal/app/provider_latency_bench.go`,
  `internal/app/app_test.go`, `docs/engineering/A21_PROVIDER_BENCHMARKS.md`,
  `docs/engineering/LATENCY_BUDGET.md`,
  `docs/engineering/OBSERVABILITY.md`, `docs/engineering/DOCTOR.md`, and
  optional `docs/engineering/PHASE7H_FAST_COMPANION_HYBRID.md`.

Acceptance gates:

- TDD red/green evidence for the v2 report shape.
- Required:
  `go test ./internal/app -run 'ProviderLatencyBench|LatencyBench|AudioFrontEnd|LocalVoiceLoopback|StackChanFastCompanion' -count=1`.
- Required:
  `go test ./internal/providers -run 'TextStream|ProviderSmoke|ProviderCatalog|AgentTask' -count=1`.
...
- Run `make verify` if shared docs/CLI changes require the wider gate.

Forbidden:

- Provider `--execute`, real provider calls, V21 execute, Gateway runtime or
  service startup, durable reports with payload bodies, firmware/NVS/flash/raw
  upload/serial writes, real `/v1/devices/control`, physical device paths,
  production dependencies, secrets, prompt/transcript/provider output/
...
- Review result: no P0 findings and no P1 findings.
- Review P2 finding: `downlink_first_frame_ms` remained in `samples` and
  `summary`, but the v2 machine-readable `canonical_metrics` and
  `stage_availability` blocks did not cover that legacy emitted key.
- Control fix:
  `9fc73de fix(app): cover legacy downlink metric in latency report`.
- The fix keeps `downlink_first_frame_ms` as a legacy compatibility alias,
  adds it to `stage_availability`, adds it to `canonical_metrics`, documents
  the alias as placeholder-only report-contract evidence, and strengthens tests
  so every `summary` metric must have canonical and stage-availability coverage.

Review verification evidence:

- Review thread passed: `git diff --check 101d590^..5f22ecc`.
- Review thread passed:
  `go test ./internal/app -run 'ProviderLatencyBench|LatencyBench|AudioFrontEnd|LocalVoiceLoopback|StackChanFastCompanion' -count=1`.
- Review thread passed:
  `go test ./internal/providers -run 'TextStream|ProviderSmoke|ProviderCatalog|AgentTask' -count=1`.
...
- Review thread passed: `go run ./cmd/a21 namespace-audit`.
- Review thread passed: `make verify`.
- Review thread passed `go run ./cmd/a21 doctor` with firmware/tooling warnings
  only.
- Review thread `go run ./cmd/a21 preflight` was blocked by reserved localhost
  ports already in use in that detached worktree environment. Control tower
  rechecked the main worktree after the P2 fix and passed `preflight`.
- Review thread `go run ./cmd/a21 promotion-readiness` reported the expected
  detached-worktree external-promotion block; control tower rechecked the main
  worktree after the P2 fix and still found local review readiness with
  external promotion blocked only by missing remote/target branch.

Control verification after the P2 fix:

- Red test evidence:
  `go test ./internal/app -run 'ProviderLatencyBenchV2ReportsMetricShapeContract' -count=1`
  failed before the fix because `canonical_metrics` missed summary metric
  `downlink_first_frame_ms`.
- Green test evidence: the same focused test passed after the fix.
- Verification passed:
  `go test ./internal/app -run 'ProviderLatencyBench|LatencyBench|AudioFrontEnd|LocalVoiceLoopback|StackChanFastCompanion' -count=1`.
- Verification passed:
  `go test ./internal/providers -run 'TextStream|ProviderSmoke|ProviderCatalog|AgentTask' -count=1`.
...
- Verification passed:
  `go run ./cmd/a21 provider-latency-bench --provider mock --mode host_loopback --iterations 2`
  with a structured `jq` check proving every `summary` key has both
  `canonical_metrics` and `stage_availability` coverage, and all execution
  flags remain false.
- Verification passed: `go run ./cmd/a21 namespace-audit`.
- Verification passed: `git diff --check`.
- Verification passed: `make verify`.
...
- Audit result: no new P0/P1/P2 regression after the Provider Latency Report
  v2 closure. Provider latency report and fixture sidecar work are closed to
  T1/T2 report-contract evidence; real ASR/provider/TTS/downlink/device/
  barge-in measurements remain intentionally unmeasured placeholders.
- Audit P0: Fast Companion Gateway boundary exists, but the local audio
  front-end still depends on the RMS baseline and plan-only mature reuse
  guidance. WebRTC APM, provider-side VAD, Silero VAD, and A21 RMS VAD need a
  comparable evidence contract before any production AEC/VAD or Opus media
  work is promoted.
- Audit P0: Professional V21 adapter evidence remains guarded; real V21
  execute stays a separate T4 window and is not opened by this audit.
- Audit P1: binary Opus/media transport planning remains important, but should
  follow the audio-front-end evidence contract so later media evidence can be
  compared through stable VAD/AEC/echo/barge-in metrics.
- Audit P1/P2: StackChan physical acceptance and hardware diagnostics remain
  T6/T7/T8 surfaces and are not opened by this audit.
- Audit P2: AgentTask remains an explicit user-selected Agent I/O Layer
  scaffold, not routing/runtime. Do not advance AgentTask runtime from this
  decision.

Decision:

- Accept the audit recommendation from thread
  `019e7c80-f077-78c1-8962-58c53bb1779e`, with supporting convergence from
  `019e7c81-9963-74d1-b860-f6cebd73ed6f`.
- Open one narrow T1/T2 implementation thread for
  `Fast Companion Audio Front-End Candidate Report Hardening`, also recorded
  as `Fast Companion Audio Front-End Evidence Contract / VAD-AEC Adapter
  Shape`.
- The implementation slice must harden report/plan/evaluation evidence shape
  only. It must not introduce native WebRTC/ESP-SR/Silero runtime, binary Opus
  transport, provider execution, V21 execution, Gateway runtime, AgentTask
  runtime, hardware control, firmware writes, production dependencies, or
  payload persistence.

Audit validation evidence:

- Thread `019e7c80-f077-78c1-8962-58c53bb1779e` passed `git diff --check`,
  `go run ./cmd/a21 namespace-audit`, focused app/provider/gateway tests,
  `go run ./cmd/a21 preflight`, and `go run ./cmd/a21 doctor`.
- The audit worktree `promotion-readiness` result was non-ready only because
  the review worktree was detached and the repository has no configured
  remote, target remote, or target branch; this does not block the control
  tower from opening the next T1/T2 implementation slice.
- No audit thread edited files, committed, pushed, executed providers or V21,
  started Gateway/runtime services, generated durable payload reports, touched
  firmware/NVS/flash/raw-upload/serial paths, called `/v1/devices/control`, or
  used physical device paths.

### Fast Companion Audio Front-End Evidence Contract Implementation Thread

Opened by the control tower after accepting the post-latency PRD next-slice
audit.

Evidence:

- Implementation thread: `019e7c88-90c4-75f1-ba01-2f5bcc5bef90`.
- Implementation thread title:
  `A21 Audio Front-End Evidence：Implementation`.
- Implementation worktree:
  `/Users/jiyurun/.codex/worktrees/7d61/New project`.
- Starting branch: `codex/a21-integration-governance-slices`.
- Starting control HEAD:
  `2b4f2ad docs(control): open post-latency prd audit`.
- Target branch: `codex/a21-audio-front-end-evidence-contract`.
- Scope: TDD hardening of `audio-front-end-plan` and
  `audio-front-end-eval` into a PRD-comparable Fast Companion VAD/AEC
  candidate evidence contract. This is report-contract and adapter-shape work,
  not production VAD/AEC/full-duplex/barge-in acceptance.
- Expected candidate/adaptor fields: `webrtc_apm`, `provider_side_vad`,
  `silero_vad`, and `a21_rms_vad`; each must expose current status,
  deployment target, availability, placeholder state, placeholder reason, and
  required evidence.
- Expected evidence categories: mock benchmark preservation, labelled fixture
  preservation, office-noise benchmark, speaker-to-mic echo report, speech
  start/end lag, barge-in stop timing, first-audio waterfall impact, CPU/
  memory profile, and metrics continuity.
- Expected files: `internal/audio/frontend_plan.go`,
  `internal/audio/frontend_eval.go`, `internal/audio/frontend_eval_test.go`,
  optional `internal/app/app_test.go`,
  `docs/engineering/PHASE7G_AUDIO_FRONT_END_EVALUATION.md`,
  `docs/engineering/A21_MATURE_VOICE_REUSE.md`,
  `docs/engineering/OBSERVABILITY.md`,
  `docs/engineering/LATENCY_BUDGET.md`, and optional
  `docs/engineering/DOCTOR.md`.
- Maximum tier: T1/T2.

Acceptance gates:

- TDD red/green evidence for the audio-front-end evidence contract.
- Required:
...
- Required: `go run ./cmd/a21 audio-front-end-plan`.
- Required: `go run ./cmd/a21 audio-front-end-eval --mock`.
- Required:
  `go run ./cmd/a21 provider-latency-bench --provider mock --mode host_loopback --iterations 2`.
- Required: `go run ./cmd/a21 namespace-audit`.
- Required: `git diff --check`.
- Run `make verify` if shared CLI or docs are touched.

Forbidden:

- Provider `--execute`, real provider calls, V21 execute, Gateway runtime or
  service startup, durable reports with raw audio or payload bodies, firmware/
  NVS/flash/raw upload/serial writes, real `/v1/devices/control`, physical
  device paths, production dependency additions, secrets, prompt/transcript/
  provider output/reasoning/raw audio/base64 audio/full URL/proxy/local path
  leakage, AgentTask runtime, binary Opus implementation, and native WebRTC/
  ESP-SR/Silero dependency implementation.
...
- Passed: `make verify`.
- Passed: `go run ./cmd/a21 preflight`.
- `go run ./cmd/a21 doctor` first reported transient reserved-port occupancy
  on `127.0.0.1:21080`, `21095`, `21114`, and `21434`; immediate `lsof`
  found no listener on those reserved ports, and the immediate rerun passed
...
- Audit worktree state: clean detached HEAD at
  `3ed7ffd docs(control): close audio front-end post review`.
- Current branch context: `codex/a21-integration-governance-slices`.
- Content gates passed: `git diff --check`, `go run ./cmd/a21 namespace-audit`,
  `go run ./cmd/a21 preflight`, rerun `go run ./cmd/a21 doctor`,
  `go run ./cmd/a21 audio-front-end-plan`,
  `go run ./cmd/a21 audio-front-end-eval --mock`, and
  `go run ./cmd/a21 provider-latency-bench --provider mock --mode host_loopback --iterations 2`.
- Environment-only findings: `promotion-readiness` was non-ready in the
  detached worktree because of detached HEAD and missing remote/target branch.
  The first doctor run saw transient reserved-port blockers that disappeared
  on rerun, matching earlier ledgered environment noise.
- P0 residual: real ASR/provider/TTS/downlink/playback/barge-in metrics remain
  placeholders and require future explicit execution windows.
- P0 residual: WebRTC APM, ESP-SR, Silero, and provider-side VAD remain
  unavailable placeholders after the audio-front-end evidence contract.
- P1 gap: A21 has no binary Opus/media transport planning contract; current
  audio evidence still uses JSON/base64 PCM fixtures and mock/host-only
  reports.
- P1/P2 future gaps: professional V21 execute, StackChan physical acceptance,
  and hardware diagnostics remain separate guarded windows.
- P2 future gap: AgentTask remains explicit user-selected Agent I/O scaffold
  only; no routing or runtime follow-up should be opened from this audit.

Decision:

- Accept the audit recommendation from
  `019e7ca2-7551-7852-8c66-5bcc76d449bb`.
- The next implementation slice is
  `A21 Binary Opus Media Transport Planning Contract`.
- The slice must remain T1/T2 planning/contract work. It must not implement
  binary Opus runtime, add native codec dependencies, start Gateway/runtime,
  execute providers or V21, write firmware/NVS/flash/serial paths, call real
  `/v1/devices/control`, use physical device paths, or persist payloads.
- Suggested file scope:
  `docs/engineering/PROTOCOL.md`,
  `docs/engineering/A21_MATURE_VOICE_REUSE.md`,
  `docs/engineering/LATENCY_BUDGET.md`,
  `docs/engineering/OBSERVABILITY.md`, `docs/engineering/DOCTOR.md`, and a
  new `docs/engineering/PHASE7I_BINARY_OPUS_MEDIA_TRANSPORT.md`. Optional code
  scope is allowed only for a plan-only CLI/report shape in `internal/protocol`
  and `internal/app` with tests.

Control-tower next gate:

- Open one narrow no-commit implementation thread for the binary Opus media

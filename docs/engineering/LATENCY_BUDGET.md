...
The Gateway audio ingress path now has a small bounded frame buffer and an explicit VAD detector boundary with deterministic RMS default. It records `audio.ingress.buffered`, `vad.speech.start`, and `vad.speech.end` trace markers and exposes ingress/VAD Prometheus metrics. This is a control-point and observability baseline for future tuning; it is not production VAD, echo cancellation, full-duplex validation, or LAN jitter characterization.

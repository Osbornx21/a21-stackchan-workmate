...
`go run ./cmd/a21 audio-front-end-eval --mock` now preserves the deterministic RMS baseline as an explicit report. It is a synthetic fixture only, but it gives future WebRTC APM, provider-side VAD, and neural VAD spikes a report shape to match before any production promotion.

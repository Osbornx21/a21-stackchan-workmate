...
`stackchan-fast-companion-turn` is the M3-prep vertical receipt. In the default mode it composes the current host-side `local-voice-loopback` evidence with real StackChan downlink delivery. With `--listen-source stackchan_mic`, it first commands the real StackChan into a bounded `LISTENING` capture window, waits until same-session physical mic frames actually arrive, consumes them through Gateway's loopback-only recent-audio capture, builds a temporary 16 kHz mono PCM WAV for the ASR boundary, then deletes the temporary raw-audio file after the turn.

```text
host fixture or physical StackChan mic evidence
...
The default `listen_source` is `host_fixture`, so the report is not an M3 pass even when all chunks are delivered. It sets `m3_candidate=false` and records a finding explaining that the listen source is not physical StackChan microphone evidence. `--listen-source stackchan_mic` is allowed only when Gateway returns same-session physical mic frames; an empty capture blocks the run and no synthetic answer playback is delivered. A true M3 candidate still requires physical mic evidence with at least 400 ms of captured device audio, a non-mock ASR provider, executed text stream, stable acknowledgement/answer playback, and the latency budget in the same traced run.
...
The report records local acknowledgement first-audio timing, answer first-audio timing, playback chunk/batch counts, playback clear status, physical mic frame count, physical mic byte count, captured audio duration, requested capture window, max RMS, temporary WAV basename, transport, trace/session ids, device identity, M3-candidate status, and findings. It does not record input text, ASR transcript, provider output text, local acknowledgement text, provider credentials, auth headers, proxy values, full provider URLs, full model paths, full WAV fixture paths, raw audio, or `data_base64`.
...
Device playback uses an initial 160 ms preroll batch followed by 80 ms steady batches. This keeps a small cushion in the StackChan playback buffer so HTTP, WebSocket, JSON parsing, and firmware loop jitter do not turn otherwise valid TTS WAVs into intermittent, electrical-sounding playback. A human-audible speaker pass is still required before claiming TTS quality.

...
Phase 7: low-latency and full-duplex optimization. Current baseline includes bounded audio ingress buffering, RMS-based mock VAD transitions, audio-path barge-in cancellation, and provider-neutral realtime audio uplink forwarding for providers that expose an explicit realtime session interface. This still does not claim production VAD, AEC, hardware full-duplex, real provider response reads, or physical-device latency acceptance. Next slices should add provider event reads, first-audio waterfall spans, mature VAD/AEC adapter evaluation, and real StackChan mic/speaker acceptance.

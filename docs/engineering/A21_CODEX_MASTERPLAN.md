...
Phase 5: StackChan firmware MVP. Current baseline includes A21-only CoreS3 firmware identity, Wi-Fi config/runtime guards, control/audio WebSocket transport probes, reconnect/local fallback state, semantic motion runtime with Y-axis servo clamp safety, build/package/upload/device-identity dry-run discipline, and native tests. Real mic capture/playback, calibrated servo hardware output, LED/touch runtime, OTA, and physical flashing remain future work.

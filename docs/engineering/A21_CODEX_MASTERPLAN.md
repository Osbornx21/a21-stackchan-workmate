...
Phase 4: provider adapters. Current baseline includes mock and cascade voice providers, provider-neutral health status, explicit cancel reason/stream acknowledgements, OpenAI/Doubao realtime adapter boundaries, selected-provider doctor health, redacted realtime plans, an explicit Gateway runtime provider guard, and offline realtime fixture smoke through fake WebSocket dialers. Gateway remains mock by default; `A21_GATEWAY_VOICE_PROVIDER=selected` is required before the provider selected by `A21_PROVIDER_PRIMARY` enters Gateway runtime. Next slices should add true realtime session entrypoints, latency metrics, and provider-specific cancellation adapters behind this boundary.

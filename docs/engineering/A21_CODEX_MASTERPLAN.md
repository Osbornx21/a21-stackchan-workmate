...
Status: v0.4, aligned with `docs/prd/A21_PRD.md` and the current Go-first A21 foundation.
...
The current product directive is `docs/prd/A21_PRD.md` v0.4. It changes the active development mainline toward Provider Spine, mainland-latency-controlled text streaming, and the fast companion hybrid lane. It does not authorize a Go-first rewrite, firmware discipline rollback, provider-key leakage, or StackChan capability reduction.
...
## Provider Spine Policy

Providers are resources, not architecture. A21 now treats provider integration as a profile-driven spine with separate lanes for `text_stream`, `voice_realtime`, `voice_hybrid`, `agent_task`, and `local_audio`. SiliconFlow, DeepSeek, StepFun, Bailian/DashScope, Moonshot, Volcengine Ark, local Ollama/vLLM, OpenAI realtime, Doubao realtime/TTS realtime, Hermes, and MiMo are candidate profiles only after they satisfy A21 redaction, latency, and lane-boundary rules. Baidu and Huawei are explicitly blocked candidates.

Provider adapters must enter through provider contracts with:
...
Phase 4: Provider Spine / text stream hot plug. Current baseline includes mock and cascade voice providers, provider-neutral health status, explicit cancel reason/stream acknowledgements, OpenAI/Doubao realtime adapter boundaries, selected-provider doctor health, redacted provider smoke evidence reports, redacted realtime plans for OpenAI realtime, Doubao speech-to-speech, and Doubao realtime TTS, an explicit Gateway runtime provider guard, offline realtime fixture smoke through fake WebSocket dialers, provider-neutral Gateway realtime session start/cancel endpoints with trace markers and latency metrics, provider audio delta mapping into A21 `audio.playback.chunk` envelopes, OpenAI realtime WebSocket output reads mapped back into A21 `VoiceEvent` downlink events, and a health-checkable Doubao S2S voice provider boundary. The current P0 Provider Spine slice is intentionally narrower than the full PRD candidate universe: the executable registry exposes only `mock` and `deepseek`, with DeepSeek as the single route-eligible `text_stream` profile using OpenAI-compatible streaming parser, `provider-smoke --stream --repeat`, and redacted first-byte/first-content timing reports. Baidu/Huawei remain blocked. Profile overrides and additional mainland candidates such as SiliconFlow, StepFun, Bailian/DashScope, Moonshot, Volcengine Ark, local Ollama, and local vLLM are deferred until M0-M3 vertical evidence is clean. Gateway remains mock by default; `A21_GATEWAY_VOICE_PROVIDER=selected` is required before the provider selected by `A21_PROVIDER_PRIMARY` enters runtime. Professional mode is kept out of opaque realtime fast paths and must use the V21 evidence path.

Phase 5: StackChan firmware MVP. Current baseline includes A21-only CoreS3 firmware identity, repository-local pinned PlatformIO bootstrap, Wi-Fi config/runtime guards, control/audio WebSocket transport probes, full 640-byte mock audio-frame uplink, guarded mic PCM frame queueing and audio-frame uplink, mock Gateway audio downlink chunks, bounded firmware playback buffering, guarded M5 speaker pump playback, microphone capture policy with speaker-busy guard, reconnect/local fallback state, semantic motion runtime with Y-axis servo clamp safety, semantic RGB state runtime, semantic touch intent runtime, playback start/stop/clear state machine, semantic StackChan capability reporting through Gateway device registry, build/package/upload/device-identity dry-run discipline, release index plus per-artifact manifest guards, latest same-commit upload guard, timestamped no-flash flash-plan receipts, office handoff/preflight/acceptance receipts, identity-only StackChan physical presence acceptance, standardized no-flash physical evidence report generation, Gateway-derived physical evidence prefill for audio frame/audio downlink/render state/touch source, no-flash per-capability physical evidence acceptance for microphone/speaker/screen/screen_touch/top_touch/servo_y/RGB, PlatformIO build-provenance checks tied to `firmware/stackchan/.pio/build/a21_stackchan_cores3/firmware.bin`, and native tests. Capability reporting now preserves the full StackChan hardware surface: microphone, speaker, screen, screen touch, top touch, Y-axis servo, planned second servo axis, RGB, camera, IMU, ambient light, proximity, battery, NFC, and infrared. Physical microphone/speaker/screen/touch/servo/RGB acceptance now has a report gate but still requires real office evidence; camera/IMU/sensor/battery/NFC/infrared tracks remain planned until implementation and acceptance prove an A21 experience that is not worse than the original StackChan behavior. OTA, production latency/full-duplex acceptance, and physical flashing remain future work.

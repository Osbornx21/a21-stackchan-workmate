...
- `capabilities`: semantic StackChan hardware capability map. Current simulator reports `microphone`, `speaker`, `screen`, `screen_touch`, `top_touch`, `servo_y`, and `rgb` as `available`. Current physical CoreS3 release firmware reports `microphone` as `disabled_m5unified_i2s_stop_crash_guard`; the isolated `a21_stackchan_cores3_mic_probe` build reports `diagnostic_probe_m5unified_i2s_capture` and must not be treated as production microphone availability. The other visible/control surfaces remain reported as `available` when initialized.
- `touch_source`: `screen` or `top_sensor` for semantic touch-origin events
- `runtime_echo`: device-applied runtime echo map. Current firmware reports `screen` as the applied render state, `servo_y` as the clamped applied Y-axis angle, and `rgb` as the applied RGB color. Diagnostic mic-probe firmware also reports string counters such as `mic_frames_captured`, `mic_driver_errors`, skip counters, mic queue depth, and `audio_ws_sent_audio_frames` so physical microphone bring-up can distinguish capture, queue, and WebSocket send failures. Gateway records this for diagnostics and capability evidence derivation, but it is still not a substitute for physical operator observation.

...
Current integration checkpoint: `a0358f2 merge: wake physical proof recorder`
...
- Current HEAD after this wake-physical-proof checkpoint:
  `a0358f2 merge: wake physical proof recorder`
...
- Current wake-word state: `wake_word.product_ready=false` and
  ...
  `wake-word-physical-proof` recorder and `wake-word-physical-acceptance`
  command/report seam are now implemented, but a real
  guarded-flash/operator physical proof report is still required before
...
- Latest wake/readiness targeted verification:
  ...
  passed on current mainline after merging the wake physical proof recorder.
  A temporary fixture-only CLI smoke for
  `go run ./cmd/a21 wake-word-physical-proof ... --output-dir <tmp>` followed
  by `go run ./cmd/a21 wake-word-physical-acceptance --package-report <fixture>
  --proof-report <tmp-proof> --output-dir <tmp>` emitted
  `a21.wake_word_physical_proof.v1` and
  `a21.wake_word_physical_acceptance.v1`; the temp reports were removed and are
...
- Latest server-side bundle refresh:
  ...
  wrote `a21-server-side-readiness-bundle-20260602-082437.json`, with
  `provider.ready=true`, `v21.ready=true`, `host_voice.ready=true`,
  `wake_word.ready=false`, wake source
  `a21-wake-word-firmware-package-20260602-075112-1780357872711914000.json`,
  collection command chaining `wake-word-physical-proof` into
  `wake-word-physical-acceptance`, and no PRD fake-green.
- Latest product-readiness refresh after the wake physical proof recorder:
  ...
  wrote `a21-product-readiness-20260602-082437.json`, with
...
| Wake physical proof recorder | `019e85b1-b35d-7421-a00e-87d5c6e7b1ec` | `/Users/jiyurun/.codex/worktrees/5edd/New project` | `codex/a21-wake-physical-proof-20260602` | Operator-facing `a21.wake_word_physical_proof.v1` recorder for guarded hardware windows | Worker stopped after red tests; control tower reclaimed, implemented, verified, and merged via `a5354bb`/`a0358f2`; do not duplicate |
...
3. Wake-word package closure: no-hardware custom package evidence now exists for
   ...
   `wake-word-physical-proof --physical-device-online --firmware-flash-executed --guarded-flash-report <guarded-flash.json> --operator-observed --wake-phrase-matched --false-wake-rejected --stock-wake-rejected --output-dir reports`
   and accepted with
   `wake-word-physical-acceptance --package-report reports/a21-wake-word-firmware-package-20260602-075112-1780357872711914000.json --proof-report reports/a21-wake-word-physical-proof-*.json --output-dir reports`.
...
| Wake word | No-hardware config/plan/build-receipt/custom package path, proof recorder, and physical acceptance seam done; product-ready still pending | Frontend/Gateway desired phrase persistence, pending firmware status, guarded plan/package commands, package report ingestion, missing build-dir/receipt diagnostic reports, explicit build receipt bridge, current reviewed `小阿二一` MultiNet package artifact and sha, `wake-word-physical-proof`, `wake-word-physical-acceptance` report ingestion/latest matching | Real guarded flash plus physical custom wake proof report |

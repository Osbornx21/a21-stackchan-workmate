...
- `internal/providers`: provider-neutral voice contracts, deterministic mock/cascade behavior, explicit provider HTTP network policy, and provider readiness registry.
...
- Provider-neutral mock/cascade contracts include health status, provider HTTP network policy, provider readiness registry, a Gateway provider health endpoint, and explicit cancel reason/stream acknowledgements; no real provider adapters exist yet.

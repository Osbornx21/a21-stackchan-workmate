...
| MCP brightness/theme/info | `self.screen.set_brightness`, `self.screen.set_theme`, `self.screen.get_info` | Live-whitelisted through `/v1/xiaozhi/mcp-control`; named product-operation endpoints are implemented locally for device status, screen brightness, screen theme, and MCP capabilities; not product-accepted | Low-medium | `T-STACKCHAN-OFFICIAL-MCP-STATUS-PARITY-001` | Endpoint tests with allowed tools only, bounded arguments, redacted send markers, and no raw body logging; live ECS evidence still needed for named endpoints | Remove endpoint mapping if raw result, hidden claim, or unsafe argument leaks |

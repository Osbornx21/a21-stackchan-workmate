...
| Servo Y/pitch | Pitch servo angle/torque/motion with clamps | Official action plan maps state/motion to clamped pitch packets and metadata; physical behavior still report-scoped | Keep and verify | `T-STACKCHAN-OFFICIAL-ACTION-PARITY-001` plus physical evidence window | Operator or instrumented pose evidence with clamp verification | Preserve current pitch-only safe mapping |
| Servo X/yaw | Yaw servo angle/PWM/continuous rotation mode | Official action plan marks yaw as `servo_x_candidate_yaw_sequence`; not product-available | Medium-high safety gate | `T-STACKCHAN-OFFICIAL-ACTION-PARITY-001` plus physical evidence window | Mechanical range, torque, clamp, and collision safety evidence | Keep `servo_x=planned_continuous_rotation_axis` |
| RGB | 12 LED individual/all control | Semantic RGB labels are recorded as `*_no_rgb_frame`; rich official per-LED API is not exposed | Medium Gateway/action mapping | `T-STACKCHAN-OFFICIAL-ACTION-PARITY-001` plus future RGB packet/evidence cut | Visible RGB evidence and semantic mapping tests | Keep current simple expression mapping |
...
| MCP brightness/theme/info | `self.screen.set_brightness`, `self.screen.set_theme`, `self.screen.get_info` | Live-whitelisted through `/v1/xiaozhi/mcp-control` as low-risk control contract; not product-accepted | Low-medium | `T-STACKCHAN-OFFICIAL-MCP-STATUS-PARITY-001` | Endpoint tests with allowed tools only, bounded arguments, redacted send markers, and no raw body logging | Remove endpoint mapping if raw result, hidden claim, or unsafe argument leaks |

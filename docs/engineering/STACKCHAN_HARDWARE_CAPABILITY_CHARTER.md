...
- A21 runtime/evidence paths know `screen`, `screen_touch`, `top_touch`,
  `servo_y`, and `rgb` as stable declared surfaces. Screen tap, top tap,
  top directional swipes, and top-touch barge-in now have 2026-06-04
  product-lane physical touch reports; other surfaces still require per-report
  acceptance rather than automatic product promotion.
...
| Screen touch | Official display touch can enter listen/config flows | Product-lane physical `screen_touch` report passed; Gateway maps accepted touch into bounded LED/head MCP reactions behind a separate product gate | Low-medium physical gate | `T-STACKCHAN-OFFICIAL-TOUCH-ACTION-EVIDENCE-001` plus `T-STACKCHAN-OFFICIAL-TOUCH-BODY-REACTION-001` | `reports/a21-stackchan-touch-acceptance-20260604-222700.json`, trace/session/device ids, source `screen`; touch reaction evidence `reports/a21-stackchan-touch-reaction-evidence-20260604-224756.json` on trace `a21-trace-44-1b-f6-e2-6a-60` | Keep touch bridge behind product allowance; disable touch reactions independently if body feedback regresses |
| Top touch | Three-zone press/release/swipe gestures | Product-lane physical top tap, swipe forward/backward, and barge-in reports passed; Gateway maps top touch into bounded LED/head MCP reactions; directional swipes still need product affordance | Low-medium physical gate | `T-STACKCHAN-OFFICIAL-TOUCH-ACTION-EVIDENCE-001` plus `T-STACKCHAN-OFFICIAL-TOUCH-BODY-REACTION-001` | `reports/a21-stackchan-touch-acceptance-20260604-222708.json`, `...222719.json`, `...222744.json`, `...222818.json` with source `top_sensor`; touch reaction evidence `reports/a21-stackchan-touch-reaction-evidence-20260604-224756.json` shows `top_swipe_backward` -> LED `120/60/0`, head `yaw=-18,pitch=24,speed=200` | Keep top touch bridge behind product allowance; disable touch reactions independently; keep directional swipe UX guided until labeling/training lands |
...
| MCP robot head/LED | `self.robot.get_head_angles`, `self.robot.set_head_angles`, `self.robot.set_led_color` | Live-whitelisted through `/v1/xiaozhi/mcp-control`; deployed to ECS with numeric JSON-RPC ids and redacted inbound MCP responses; Gateway reuses the same bounded MCP tools for product-gated touch/state body reactions and exposes `/v1/xiaozhi/body-preset` as a two-step LED/head expression sequence alias | Medium physical body-control gate | `T-STACKCHAN-OFFICIAL-ROBOT-MCP-BODY-CONTROL-001`, `T-STACKCHAN-OFFICIAL-TOUCH-BODY-REACTION-001`, `T-STACKCHAN-OFFICIAL-STATE-BODY-REACTION-001`, and `T-STACKCHAN-OFFICIAL-BODY-PRESET-SEQUENCE-001` | Focused Go tests, ECS deploy, trace markers `xiaozhi.mcp.robot_led_color.sent`, `xiaozhi.mcp.robot_head_angles_set.sent`, `xiaozhi.touch_reaction.*`, `xiaozhi.state_reaction.*`, `xiaozhi.body_preset.*`, device-session `xiaozhi.mcp.response.received`, 2026-06-04 serial HAL logs, and touch reaction report `reports/a21-stackchan-touch-reaction-evidence-20260604-224756.json`; body-preset visible movement still needs physical evidence after deploy | Disable mappings or runtime gates `A21_XIAOZHI_PRODUCT_TOUCH_REACTIONS` / `A21_XIAOZHI_PRODUCT_STATE_REACTIONS` if official MCP response handling, physical movement, touch acceptance, or state expression regresses |
